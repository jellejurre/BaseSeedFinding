...
public class BastionFeatures {
    ...
    public boolean generate(CPos pos) {
        ...
        if (type.equals("housing")) {
            BPos offset =
                new BPos(-8, 0, 24).transform(BlockMirror.NONE, bastionRotation, new BPos(0, 0, 0));

            BPos chunkEdgeOfChestsRampart1 =
                pos.toBlockPos().add(offset.getX(), 0, offset.getZ()).toChunkCorner();

            rand.setDecoratorSeed(seed, chunkEdgeOfChestsRampart1.getX(),
                chunkEdgeOfChestsRampart1.getZ(), 40012, MCVersion);
            ...
            rand.setDecoratorSeed(seed, chunkEdgeOfChestsRampart2.getX(),
                chunkEdgeOfChestsRampart2.getZ(), 40012, MCVersion);
